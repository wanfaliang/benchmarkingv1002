...
router = APIRouter(prefix="/api/research/bls/ce", tags=["CE Explorer"])


@router.get("/dimensions", response_model=CEDimensions)
def get_ce_dimensions(
...
def get_ce_overview_timeline(
    months_back: int = Query(24, ge=0, le=9999, description="Number of months of history (0 for all time)"),
    current_user=Depends(get_current_user),
    db: Session = Depends(get_data_db)
):
...
def get_ce_supersector_analysis(
    current_user=Depends(get_current_user),
    db: Session = Depends(get_data_db)
):
