...
/**
 * Datasets.tsx - Professional Dataset Management Interface
 *
 * Features:
...
type ActionType = "start" | "retry" | "share" | "export" | "menu";
...
const statusConfig: Record<DatasetStatus, StatusConfigItem> = {
  created: {
    label: "Created",
    color: "slate",
    icon: Clock,
...
function StatusChip({ status, progress }: StatusChipProps): React.ReactElement {
...
function EmptyState({ onCreate, hasFilters }: EmptyStateProps): React.ReactElement {
  if (hasFilters) {
    return (
  ...
  return (
    <div className="text-center py-20">
      ...
      <p className="mt-3 text-slate-700 max-w-md mx-auto">
        Create your first dataset to explore financial data with flexible
        filtering, custom dashboards, and powerful visualizations.
      </p>
      <button
        onClick={onCreate}
...
interface DatasetCardProps {
  ...
  onAction: (action: ActionType, dataset: Dataset) => void;
}
    ...
    <article className="group rounded-2xl border border-slate-200 bg-white p-5 hover:shadow-lg hover:border-slate-300 transition-all duration-200">
      ...
      {dataset.status === "ready" && (
        <div className="mt-3 pt-3 border-t border-slate-100 flex items-center gap-2 text-xs text-slate-700">
          <span>{formatBytes((dataset.data_size_mb || 0) * 1024 * 1024)}</span>
          <span>•</span>
...
function DatasetRow({
  ...
  onOpen,
  onAction,
}: DatasetRowProps): React.ReactElement {
      ...
      <td className="px-4 py-4 text-sm text-slate-700">
        {dataset.status === "ready" ? (
          <div className="space-y-0.5">
            <div>{formatBytes((dataset.data_size_mb || 0) * 1024 * 1024)}</div>
...
export default function Datasets(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    if (collectingDatasets.length === 0) return undefined;
  ...
  const summary = useMemo<Partial<Summary>>(() => {
    if (!datasets) return {};
  ...
  const handleAction = async (
    ...
    try {
      switch (action) {
        ...
        case "export":
          ...
          try {
            ...
            alert("Export failed: " + formatError(e as Parameters<typeof formatError>[0]));
          }
          break;
        case "menu":
      ...
      alert(formatError(e as Parameters<typeof formatError>[0]));
    }
  };
