...
import { Plus, Search, Clock, CheckCircle2, AlertCircle, Trash2, Edit2, X, Check, XCircle, Loader2, Grid3x3, List, BarChart3, FileText, LucideIcon } from 'lucide-react';

/**
 * Dashboard.tsx - Professional Analysis Management Interface
 *
 * Features:
...
type AnalysisStatus = 'created' | 'collecting' | 'collection_complete' | 'generating' | 'complete' | 'failed';
...
const statusConfig: Record<AnalysisStatus, StatusConfigItem> = {
  created: {
    label: "Created",
    color: "slate",
    icon: Clock,
...
function AnalysisCard({ analysis, onOpen, onEdit, onDelete }: AnalysisCardProps): React.ReactElement {
...
function AnalysisRow({ analysis, onOpen, onEdit, onDelete }: AnalysisRowProps): React.ReactElement {
  const [isEditing, setIsEditing] = useState(false);
  const [editName, setEditName] = useState(analysis.name || 'Untitled Analysis');
...
const Dashboard: React.FC = () => {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      setBannerMessage(err.response?.data?.detail || 'Failed to resend email. Please try again.');
    } finally {
