// frontend/src/pages/WebSocketAnalysisDetail.tsx
...
/**
 * WebSocketAnalysisDetail.tsx - Analysis Detail View with WebSocket
 *
 * Features:
...
const WebSocketAnalysisDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (!analysis || isConnected) return undefined;
    ...
    if (shouldPoll) {
      console.log('Using fallback polling (WebSocket not connected)');
  ...
  const getProgressIcon = (progress: number): React.ReactElement => {
  ...
  const getStatusIcon = (status: SectionStatus): React.ReactElement => {
    ...
    const icons: Record<SectionStatus, React.ReactElement> = {
      ...
      processing: <Loader size={20} style={{ color: '#3b82f6', ...spinStyle }} />,
...
export default WebSocketAnalysisDetail;
