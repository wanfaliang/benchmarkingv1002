...
import type { AnalysisSection, SectionStatus } from '../types';

/**
 * SectionViewer.tsx - Section HTML Viewer with Sidebar Navigation
 *
 * Features:
 * - Display section HTML in iframe
...
const SectionViewer: React.FC = () => {
  const { id, sectionNum } = useParams<{ id: string; sectionNum: string }>();
  ...
  const currentSectionNum = parseInt(sectionNum || '0');
  ...
  const getStatusIcon = (status: SectionStatus): React.ReactElement => {
    ...
    if (status === 'processing') return <Loader size={size} style={{ color: '#3b82f6', ...spinStyle }} />;
