// frontend/src/pages/DatahubBuilder.tsx
...
/**
 * DatahubBuilder.tsx - Drag-and-Drop Datahub Builder
 *
 * Features:
...
type WidgetType = "chart" | "table" | "stats" | "text";
...
export default function DatahubBuilder(): React.ReactElement {
  ...
  useEffect(() => {
    if (isNewDatahub) {
      ...
      setWidgets(blank.widgets as Record<string, Widget>);
    } else if (id) {
      const loaded = getDatahub(id);
      if (loaded) {
        setDatahub(loaded);
        setLayout(loaded.layout);
        setWidgets(loaded.widgets as Record<string, Widget>);
      } else {
  ...
  const handleSave = (): void => {
    ...
    try {
      const updated: Datahub = {
        ...datahub,
        layout,
        widgets: widgets as Datahub["widgets"],
      };
      saveDatahub(updated);
  ...
  // Configure widget
  const handleConfigureWidget = (widgetId: string): void => {
            ...
            <div className="min-w-0">
              <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  setDatahub({ ...datahub, name: e.target.value })
                }
              ...
              <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  setDatahub({ ...datahub, description: e.target.value })
                }
        ...
        {showConfig && activeWidgetData && (
          <aside className="w-96 bg-white border-l border-slate-200 overflow-y-auto">
            <ConfigPanel
              ...
              onUpdate={(updates) =>
                activeWidget && handleUpdateWidget(activeWidget, updates)
              }
              onClose={() => {
                setShowConfig(false);
                setActiveWidget(null);
