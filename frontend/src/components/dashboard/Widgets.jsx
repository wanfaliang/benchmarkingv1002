...
import { BarChart3, Table as TableIcon, TrendingUp, FileText, Settings, Maximize2, RefreshCw, AlertCircle } from "lucide-react";
import { datasetsAPI, formatError } from "../../services/api";
import ChartBuilder from "../charts/ChartBuilder";

/**
 * Widget Components for Datahub Builder
 * 
 ...
 * - onRemove: callback to remove widget
...
function WidgetWrapper({ title, icon: Icon, onConfigure, onRemove, isEditing, children, error }) {
  return (
...
export function ChartWidget({ id, title, config, onConfigChange, onRemove, isEditing }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const method = config.dataSource;
  ...
  const handleConfigure = () => {
    console.log('🔧 handleConfigure called! id:', id, 'onConfigChange exists:', !!onConfigChange);
    if (onConfigChange) {
      // Open config panel - handled by parent
      onConfigChange(id);
    }
  };
...
export function TableWidget({ id, title, config, onConfigChange, onRemove, isEditing }) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
...
export function StatsWidget({ id, title, config, onConfigChange, onRemove, isEditing }) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
...
export function TextWidget({ id, title, config, onConfigChange, onRemove, isEditing }) {
  const handleConfigure = () => {
    if (onConfigChange) {
      onConfigChange(id);
    }
