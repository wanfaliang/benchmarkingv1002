...
export default function SaveQueryDialog({ 
  ...
  onClose, 
  dataSource,
  currentQuery = {},  // NEW: Receive current query state from parent
  hasFilters = false
}) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await onSave({
        ...
        query_config: currentQuery,  // Send complete query configuration
        is_public: isPublic,
      });
          ...
          <div className="px-6 py-4 space-y-4">
            ...
            <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 text-sm">
              ...
              <div className="space-y-1.5 text-xs text-blue-700">
                ...
                {(currentQuery.companies?.length > 0 || currentQuery.years?.length > 0) && (
                  <div className="mt-2 pt-2 border-t border-blue-200">
                    {currentQuery.companies?.length > 0 && (
                      <div>Companies: {currentQuery.companies.join(", ")}</div>
                    )}
                    {currentQuery.years?.length > 0 && (
