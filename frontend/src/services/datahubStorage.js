/**
 * datahubStorage.js - LocalStorage Helper for Datahubs
 * 
 ...
 */
export function getAllDatahubs() {
  try {
    const stored = localStorage.getItem(STORAGE_KEY);
    if (!stored) return [];
    
    const datahubs = JSON.parse(stored);
 ...
 */
export function saveDatahub(datahub) {
  try {
    const datahubs = getAllDatahubs();
 ...
 */
export function deleteDatahub(id) {
  try {
    const datahubs = getAllDatahubs();
 ...
 * Duplicate datahub
 */
export function duplicateDatahub(id) {
  try {
 ...
 */
export function exportDatahub(id) {
 ...
 */
export function importDatahub(file) {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    
    reader.onload = (e) => {
      try {
        const datahub = JSON.parse(e.target.result);
 ...
 */
export function clearAllDatahubs() {
 ...
 */
export function createBlankDatahub() {
  return {
    id: `datahub_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
